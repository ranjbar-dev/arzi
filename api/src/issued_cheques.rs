...
const BATCH_SORT_COLUMNS: &[(&str, &str)] = &[
...
async fn list_batches(
    ...
    let mut tx = db::begin(&state.pool, auth.tenant_id)
        .await
        .map_err(|_| internal_error())?;
    let rows: Vec<BatchRecord> = sqlx::query_as(&format!(
        "SELECT {BATCH_COLUMNS} FROM cheque_payment_batches WHERE tenant_id = $1 \
         AND ($2::bigint IS NULL OR fiscal_year_id = $2) \
         AND ($3::text IS NULL OR batch_number ILIKE '%' || $3 || '%') \
