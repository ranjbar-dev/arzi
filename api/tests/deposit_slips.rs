...
async fn posts_correctly_narrated_balanced_voucher(pool: PgPool) -> sqlx::Result<()> {
    ...
    let detail = json_body(
        get(
            &router,
            &fx.token,
            &format!("/api/v1/deposit-slips/{slip_id}"),
        )
        .await,
    )
    .await;
    let voucher_id = detail["voucherId"]
        .as_i64()
        .expect("slip must have a real voucher (B8.5-defect-6 fix)");
    ...
    .await?;
    assert_eq!(
        debit_row.0, fx.bank_account_id,
        "debit line must be the bank account"
    );
    assert_eq!(
        credit_row.0, fx.payer_account_id,
        "credit line must be the payer"
    );
    assert_eq!(
        debit_row.1, credit_row.1,
        "narration must be identical on both lines -- no swap possible"
    );
